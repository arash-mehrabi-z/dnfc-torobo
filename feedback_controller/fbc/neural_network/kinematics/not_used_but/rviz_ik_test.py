...
def main():
    ...
    torobo = Torobo(movegroup="right_arm", tooltip="right_arm/link_7")
    res = torobo.compute_fk(joint_angles=np.radians([20.0, -20.0, 50.0, 10.0, 50.0, 50.0, 50]))
    print "compute_fk returned"
    res = torobo.compute_ik(x=res[0], y=res[1], z=res[2], roll=res[3], pitch=res[4], yaw=res[5])
    print "compute_ik returned"
...
class Torobo:
    ...
    def __init__(self, movegroup, tooltip):
        ...
        # load MoveGroupCommander by string('right_arm') registerd in 'torobo.srdf'
