...
%Arm link0 frame to link1 frame
 a1 = q(ARMix(1));  %Joint 1 dependent represention in "right_arm/link_0" frame  = "torso/right_shoulder_link"
 R01  = [  sin(a1)      cos(a1)    0;
          -cos(a1)      sin(a1)    0 ;
            0           0          1 ];
 O01 = [0, 0, 0.1775];
 ...
 a2 = q(ARMix(2));  %Joint 2 dependent represention in "right_arm/link_1" frame
 R12  = [ cos(a2)   -sin(a2)       0;
          0          0             1;
         -sin(a2)   -cos(a2)       0 ];
 O12 = [0.02, 0, 0];
 ...
 a3 = q(ARMix(3));  %Joint 3 dependent represention in "right_arm/link_2" frame
 R23  = [ cos(a3)   -sin(a3)       0;
          0          0            -1;
          sin(a3)    cos(a3)       0 ];
 ...
 a4 = q(ARMix(4));  %Joint 4 dependent represention in "right_arm/link_3" frame
 R34  = [ cos(a4)   -sin(a4)       0;
          0          0             1;
         -sin(a4)   -cos(a4)       0 ];
 O34  = [0,0,0];
...
a5=q(ARMix(5));  %Joint 5 dependent represention in "right_arm/link_4" frame
...
a6=q(ARMix(6));  %Joint 6 dependent represention in "right_arm/link_5" frame
 ...
 a7=q(ARMix(7));  %Joint 7 dependent represention in "right_arm/link_6" frame
 ...
 %----
 % right_gripper/mimic_link to gripper_base=link_tip=link_7  grip aperture dependent 
 ...
 T7mim = [1 0 0 0;
          ...
          0 0 0 1];
 %----
 % right_gripper/finger_link to mimic_link  grip aperture dependent transformation
 ...
 Tmimfin = [-1 0 0 0;
             ...
             0 0 0 1];
 %---
 % right_gripper/grasping_frame to gripper_base=link_tip=link_7  fixed
...
% % % % --------------- This hack makes the left arm to look like obeyin a left-handed reference frame---- 
% % % % So not used.
% % % % This is a temporart hack. Do the propoer IK for the left arm as well!
% % % % If the JointState viewer movement is the same for each joint then
% % % % everything thing should be the same except may be wrist rotation..
% % % 
% % % ARMTIP_T_left =  ARMTIP_T;  ARMTIP_T_left(2,:) = -ARMTIP_T_left(2,:); 
% % % WRIST_T_left  =  WRIST_T;   WRIST_T_left(2,:)  = -WRIST_T_left(2,:);  
% % % 
% % % %ARMTIP_T_left = rotZ(pi, true)*ARMTIP_T;
% % % %WRIST_T_left  = rotZ(pi, true)*WRIST_T;
% % % fcode_left = gen_FKcode(ARMTIP_T_left, q, 'FK_left_ARMTIP.m');
% % % [J_left,jcode_left]  = gen_posJcode(ARMTIP_T_left, q, 'posJ_left_ARMTIP.m');
% % %  
% % % fcode = gen_FKcode(WRIST_T_left, q, 'FK_left_WRIST.m');
% % % [J,jcode]  = gen_posJcode(WRIST_T_left, q, 'posJ_left_WRIST.m');
% % %  