...
class Tester():
    ...
    def load_diffusion_model(self, train_no, epoch_no, model_complexity):
        ...
        model_name = self.config.get_model_name(
            False, False, False, use_diffusion=True
        )
        num_params = self.config.get_params_num(self.diffusion_model)
        ...
        # Also load the dataset to get normalization stats
    ...
    def get_emulated_diffusion(self, num, return_path_point=False):
        ...
        if self.diffusion_model is None:
            raise Exception("Diffusion model not loaded. Call load_diffusion_model first.")
        ...
        with torch.no_grad():
            while step < self.dataset.shape[1]:
                ...
                if action_buffer is None or action_buffer_idx >= self.config.action_horizon:
                    ...
                    # Normalize observation using dataset stats
                    from diffusion_dataset import normalize_data
                    obs_seq_norm = normalize_data(obs_seq_np, self.diffusion_dataset.stats['obs'])
                    obs_seq_norm = torch.from_numpy(obs_seq_norm).to(self.device).float()
                    obs_seq_norm = obs_seq_norm.unsqueeze(0)  # (1, obs_horizon, obs_dim)

                    # Get action sequence from diffusion model
                    action_seq_norm = self.diffusion_model.get_action(obs_seq_norm)  # (1, pred_horizon, action_dim)
                    action_seq_norm = action_seq_norm.squeeze(0).cpu().numpy()  # (pred_horizon, action_dim)
