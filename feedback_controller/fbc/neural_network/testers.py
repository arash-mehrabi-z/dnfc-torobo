...
class Tester():
    ...
    def load_model(self, train_no, epoch_no, use_custom_loss, model_complexity):
        ...
        self.model = GeneralModel(
            ...
            cont_hid=cont_hid,
            use_image=False)
        self.baseline = MLPBaseline(encoded_space_dim=self.config.encoded_space_dim,
                                    target_dim=self.target_size + self.onehot_size,
                                    ee_dim=self.ee_dim,
                                    state_dim=self.state_size,
                                    action_dim=self.joint_size,
    ...
    def get_delta_ang_offline(self, usebaseline, num):
        ...
        for i in range(299):
            ...
            if usebaseline:
                velocities_tensor, _, _, _ = self.baseline(goal_nn, ee_repr_nn, state_nn)
    ...
    def get_emulated(self, use_baseline, num, use_angle=False, return_path_point=False):
        ...
        for i in range(self.dataset.shape[1]):
            ...
            if use_baseline:
                velocities_tensor, x_des, x, _ = self.baseline(goal_nn, ee_repr_nn, state_nn)
            else:
